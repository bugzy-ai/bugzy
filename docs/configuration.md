...
| `tool` | AI coding tool (`claude-code`, `cursor`, `codex`) | string |
...
| `codex` | Experimental | OpenAI's terminal-based agent |

> **Note**: Cursor and Codex support is experimental. Some features may not work as expected.
...
✓ Updated .bugzy/config.json

Regenerating task commands...
✓ Regenerated 8 commands in .claude/commands/
...
- All task commands in `.<tool>/commands/` (e.g., `.claude/commands/`, `.cursor/commands/`)
- All subagent configs in `.<tool>/agents/`
- MCP configuration file (`.<tool>/mcp.json` or TOML for Codex)
...
Bugzy automatically generates MCP configuration based on your subagent configuration. The format varies by tool.

### Claude Code / Cursor (JSON format)

Configuration is stored in `.<tool>/mcp.json`:
